...
public final class UserPostgreSqlDAO extends SqlConnection implements UserDAO {
	...
	public List<UserEntity> findByFilter(final UserEntity filterEntity) {
	    ...
	    try (var preparedStatement = this.getConnection().prepareStatement(sql)) {
	        ...
	        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_SQL_EXCEPTION_FINDING_USER_WHILE_PREPARATION.getContent()
	                + exception.getMessage();
	        throw NoseException.create(exception, userMessage, technicalMessage);

	    } catch (final Exception exception) {
	        var userMessage = MessagesEnum.USER_ERROR_UNEXPECTED_EXCEPTION_FINDING_USER_WHILE_EXECUTION.getContent();
	        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_UNEXPECTED_EXCEPTION_FINDING_USER_WHILE_PREPARATION
	...
	private void createWhereClauseFindByFilter(final StringBuilder sql, final List<Object> parametersList,
	    ...
	    if (filterEntityValidated.getId() != null &&
	        !UUIDHelper.getUUIDHelper().isDefaultUUID(filterEntityValidated.getId())) {
	        addCondition(conditions, parametersList, true, "u.\"id\" = ", filterEntityValidated.getId());
	    }

	    
	    if (filterEntityValidated.getIdentificationType() != null &&
	        filterEntityValidated.getIdentificationType().getId() != null &&
	        !UUIDHelper.getUUIDHelper().isDefaultUUID(filterEntityValidated.getIdentificationType().getId())) {
	        addCondition(conditions, parametersList, true, "u.\"tipoIdentificacion\" = ", 
	                     filterEntityValidated.getIdentificationType().getId());
	    }

	    
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getIdentificationNumber())) {
	        addCondition(conditions, parametersList, true, "u.\"numeroIdentificacion\" = ", 
	                     filterEntityValidated.getIdentificationNumber());
	    }

	    
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getFirstName())) {
	        addCondition(conditions, parametersList, true, "u.\"primerNombre\" = ", 
	                     filterEntityValidated.getFirstName());
	    }
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getMiddleName())) {
	        addCondition(conditions, parametersList, true, "u.\"segundoNombre\" = ", 
	                     filterEntityValidated.getMiddleName());
	    }
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getLastName())) {
	        addCondition(conditions, parametersList, true, "u.\"primerApellido\" = ", 
	                     filterEntityValidated.getLastName());
	    }
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getSecondLastName())) {
	        addCondition(conditions, parametersList, true, "u.\"segundoApellido\" = ", 
	                     filterEntityValidated.getSecondLastName());
	    }

	    
	    if (filterEntityValidated.getResidenceCity() != null &&
	        filterEntityValidated.getResidenceCity().getId() != null &&
	        !UUIDHelper.getUUIDHelper().isDefaultUUID(filterEntityValidated.getResidenceCity().getId())) {
	        addCondition(conditions, parametersList, true, "u.\"ciudadResidencia\" = ", 
	                     filterEntityValidated.getResidenceCity().getId());
	    }

	    
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getEmail())) {
	        addCondition(conditions, parametersList, true, "u.\"correoElectronico\" = ", 
	                     filterEntityValidated.getEmail());
	    }

	    
	    if (!TextHelper.isEmptyWithTrim(filterEntityValidated.getCellPhoneNumber())) {
	        addCondition(conditions, parametersList, true, "u.\"numeroTelefonoMovil\" = ", 
	                     filterEntityValidated.getCellPhoneNumber());
	    }

	    
	    if (!filterEntityValidated.isEmailConfirmedDefaultValue()) {
	        addCondition(conditions, parametersList, true, "u.\"correoElectronicoConfirmado\" = ", 
	                     filterEntityValidated.isEmailConfirmed());
	    }
	    if (!filterEntityValidated.isCellPhoneNumberConfirmedDefaultValue()) {
	        addCondition(conditions, parametersList, true, "u.\"numeroTelefonoMovilConfirmado\" = ", 
	                     filterEntityValidated.isCellPhoneNumberConfirmed());
	    }
	...
	private void addCondition(final List<String> conditions, 
			final List<Object> parametersList, final boolean condition,
	...
	private List<UserEntity> executeSentenceFindByFilter(final PreparedStatement preparedStatement) {
	    ...
	    try (var resultSet = preparedStatement.executeQuery()) {
	        ...
	        while (resultSet.next()) {
	            ...
	            identificationType.setId(UUIDHelper.getUUIDHelper()
	                    .getFromString(resultSet.getString("idTipoIdentificacion")));
	            ...
	            city.setState(state);
	            city.setId(UUIDHelper.getUUIDHelper()
	                    .getFromString(resultSet.getString("idCiudadResidencia")));
	        ...
	        var userMessage = MessagesEnum.USER_ERROR_SQL_EXCEPTION_FINDING_USER_WHILE_EXECUTION.getContent();
	        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_SQL_EXCEPTION_FINDING_USER_WHILE_EXECUTION.getContent()
	                + exception.getMessage();
	        throw NoseException.create(exception, userMessage, technicalMessage);

	    } catch (final Exception exception) {
	        var userMessage = MessagesEnum.USER_ERROR_UNEXPECTED_EXCEPTION_FINDING_USER_WHILE_EXECUTION.getContent();
	        var technicalMessage = MessagesEnum.TECHNICAL_ERROR_UNEXPECTED_EXCEPTION_FINDING_USER_WHILE_EXECUTION.getContent()
	                + exception.getMessage();
	        throw NoseException.create(exception, userMessage, technicalMessage);
	    }
	...
	public UserEntity findById(final UUID id) {
	    if (ObjectHelper.isNull(id) || UUIDHelper.getUUIDHelper().isDefaultUUID(id)) {
